...
void close_hdf5_handles(const raw_data_t *df) {
    ...
    H5Tclose(df->pps.datatype);
    H5Dclose(df->pps.dataset);
...
int c_collect_data(const char *filename, const char *ip_addr, const int port) {
    ...
    //FIXME: Here, I need to change the cache size to match.. what we'll get
    ...
    df.pps.rank = H5Sget_simple_extent_dims(
    ...
    df.pps_mem_space = H5Screate_simple(1, misc_memspace_dim, NULL);
    ...
    while(!stop){
        ...
        //Select the new hyperslab for the incomming data
