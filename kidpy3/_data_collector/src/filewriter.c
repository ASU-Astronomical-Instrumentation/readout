...
int load_rawDataFile(const char *filename) {
  rawdata_t df;
  ...
  df.rank = H5Sget_simple_extent_dims(df.dataspace, df.dimms, df.maxdims);

  printf("Loaded file, DIMMS = (%ld, %ld)", df.dimms[0], df.dimms[1]);

  /* H5Dwrite(df.dset, df.datatype, H5S) */
  H5Dclose(df.dset);
...
int main(int argc, char **argv) {
  ...
  rawdata.dataspace =
      H5Screate_simple(rawdata.rank, rawdata.dimms, rawdata.maxdims);
  rawdata.dset =
